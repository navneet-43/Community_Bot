...
class RuskMediaBot(commands.Bot):
    ...
    async def on_member_join(self, member):
        ...
        embed = discord.Embed(
            ...
            description="Thank you for joining! Please complete this quick 7-question screening to get access to relevant content channels.",
            color=0x00ff00
        )
        ...
        try:
            ...
            await self.send_screening_question_dm(member, 'show_types')
            
        except discord.Forbidden:
            logger.warning(f"Could not send DM to {member}, user has DMs disabled")
            # Send in general channel instead
            general_channel = discord.utils.get(member.guild.channels, name="general-screened")
            if general_channel:
                await general_channel.send(
                    f"{member.mention} Welcome! Please check your DMs to complete the screening process. "
                    f"If DMs are disabled, please enable them temporarily to complete screening."
                )
    ...
    async def complete_screening_dm(self, member: discord.Member, user_id: int):
        ...
        embed = discord.Embed(
            ...
            description="Thank you for completing the screening process. You now have access to relevant content channels in the server!",
            color=0x00ff00
        )
        ...
        embed.add_field(
            name="What's Next?",
            value="Head back to the Rusk Media server! You now have access to content channels based on your preferences.",
            inline=False
        )
    ...
    @app_commands.command(name="start_screening", description="Start the screening process (backup if auto-screening fails)")
    async def start_screening(self, interaction: discord.Interaction, campaign: str = "MANUAL_START"):
        """Manual screening start - backup option"""
